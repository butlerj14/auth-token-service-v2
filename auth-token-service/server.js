...
app.post('/get-auth-token', async (req, res) => {
  ...
  try {
    const result = await pool.query('SELECT api_key FROM clients WHERE client_id = $1', [client_id]);
...
app.post('/add-client', async (req, res) => {
  const { client_id, api_key, admin_secret } = req.body;
  ...
  if (!client_id || !api_key) {
    return res.status(400).json({ error: 'Missing client_id or api_key' });
  }

  try {
    await pool.query(
      'INSERT INTO clients (client_id, api_key) VALUES ($1, $2) ON CONFLICT (client_id) DO UPDATE SET api_key = $2',
      [client_id, api_key]
    );
